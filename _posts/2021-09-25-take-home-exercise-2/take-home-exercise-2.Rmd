...
### 8.2.3 Plotting Kernel Density Maps using tmap

Similarly, we will need to convert the respective KDE outputs into **Gridded KDE outputs** before converting them into **RasterLayer** objects.

```{r echo=TRUE}
kde_airbnb2019_ppl_pr_raster <- bw_ppl(airbnb2019_SG_ppp_pr) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2019_ppl_en_raster <- bw_ppl(airbnb2019_SG_ppp_en) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2019_ppl_sh_raster <- bw_ppl(airbnb2019_SG_ppp_sh) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2021_ppl_pr_raster <- bw_ppl(airbnb2021_SG_ppp_pr) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2021_ppl_en_raster <- bw_ppl(airbnb2021_SG_ppp_en) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2021_ppl_sh_raster <- bw_ppl(airbnb2021_SG_ppp_sh) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2021_ppl_ho_raster <- bw_ppl(airbnb2021_SG_ppp_ho) %>%
  as.SpatialGridDataFrame.im() %>%
  raster()

kde_airbnb2019_ppl_pr_raster
kde_airbnb2019_ppl_en_raster
kde_airbnb2019_ppl_sh_raster

kde_airbnb2021_ppl_pr_raster
kde_airbnb2021_ppl_en_raster
kde_airbnb2021_ppl_sh_raster
kde_airbnb2021_ppl_ho_raster
```

From the output messages, we can observe that the coordinate reference systems for all **RasterLayer** objects are **NA**. We have to assign them coordinate reference systems in order to plot them using **tmap** package.

```{r echo=TRUE}
projection(kde_airbnb2019_ppl_pr_raster) <- CRS("+init=EPSG:3414")
projection(kde_airbnb2019_ppl_en_raster) <- CRS("+init=EPSG:3414")
projection(kde_airbnb2019_ppl_sh_raster) <- CRS("+init=EPSG:3414")

projection(kde_airbnb2021_ppl_pr_raster) <- CRS("+init=EPSG:3414")
projection(kde_airbnb2021_ppl_en_raster) <- CRS("+init=EPSG:3414")
projection(kde_airbnb2021_ppl_sh_raster) <- CRS("+init=EPSG:3414")
projection(kde_airbnb2021_ppl_ho_raster) <- CRS("+init=EPSG:3414")
```
...
We will not be plotting the raster map for `hotel` Airbnb 2021 listings because there is no such room type in Airbnb 2019 listings, so there is no point for comparison.

```{r echo=TRUE}
raster_compare <- function(df1, title1, df2, title2){
  tmap_mode('view')
  
  tm_basemap("OpenStreetMap") + 
    tm_shape(df1) + 
      tm_raster("v", title=title1, alpha=0.5, palette="Reds") +
    tm_shape(df2) + 
      tm_raster("v", title=title2, alpha=0.45, palette="Blues")
}
```

```{r echo=TRUE}
raster_compare(kde_airbnb2019_ppl_raster, "Airbnb2019 Listings",
               kde_airbnb2021_ppl_raster, "Airbnb2021 Listings")
```

```{r}
tmap_mode('plot')
```
The following can be observed from the plotted raster maps:

- Shades of red represent Airbnb Listings in 2019 while shades of blue represent Airbnb Listings in 2021.
- Both Airbnb Listings in 2019 and 2021 are concentrated in the south of the study area.
- The concentration of Airbnb listings in 2021 is lower than the concentration of Airbnb listings in 2019, which can be seen from the fewer areas of blue as compared to the shades of red. 

```{r echo=TRUE}
raster_compare(kde_airbnb2019_ppl_pr_raster, "Airbnb2019 Private Rooms",
               kde_airbnb2021_ppl_pr_raster, "Airbnb2021 Private Rooms")
```
...
- Shades of red represent `private room` Airbnb Listings in 2019 while shades of blue represent `private room` Airbnb Listings in 2021.
- We can see that the concentration of `private room` Airbnb listings is generally low for both years.
- Both `private room` Airbnb listings in 2019 and 2021 are concentrated in the south.
- The concentration of `private room` Airbnb listings in 2021 is lower than the concentration of `private room` Airbnb listings in 2019, which can be seen from the fewer areas of blue as compared to the shades of red. 

```{r echo=TRUE}
raster_compare(kde_airbnb2019_ppl_en_raster, "Airbnb2019 Entire Home/Apt",
               kde_airbnb2021_ppl_en_raster, "Airbnb2021 Entire Home/Apt")
```
...
- Shades of red represent `entire home/apt` Airbnb Listings in 2019 while shades of blue represent `entire home/apt` Airbnb Listings in 2021.
- We can see that there is considerable concentration of `entire home/apt` Airbnb listings for both years.
- Both `entire home/apt` Airbnb listings in 2019 and 2021 are concentrated in the south.
- The concentration of `entire home/apt` Airbnb listings in 2021 looks to be higher than the concentration of `entire home/apt` Airbnb listings in 2019, which can be seen from the fewer areas of red as compared to the shades of blue. 

```{r echo=TRUE}
raster_compare(kde_airbnb2019_ppl_sh_raster, "Airbnb2019 Shared Rooms",
               kde_airbnb2021_ppl_sh_raster, "Airbnb2021 Shared Rooms")
```
...
- Shades of red represent `shared room` Airbnb Listings in 2019 while shades of blue represent `shared room` Airbnb Listings in 2021.
- Both `shared room` Airbnb listings in 2019 and 2021 are concentrated in the south.
- The concentration of `shared room` Airbnb listings in 2021 is observed to be higher than the concentration of `shared room` Airbnb listings in 2019, which can be seen from the fewer areas of red as compared to the shades of blue. 

### 8.3 Second-order Spatial Point Pattern Analysis

We cannot conclude that the distribution of Airbnb listings by room types are impacted by COVID-19 from the kernel density maps alone. We therefore need to employ second-order spatial point patterns analysis to draw statistical conclusions. 

### 8.3.1 Extract Point Data in Study Area

Since most Airbnb 2019 and 2021 listings are concentrated around the `Downtown Core` planning area, we are only going to extract data points within it for the second-order spatial point pattern analysis. 

```{r echo=TRUE}
airbnb2019_dt_ppp_pr = airbnb2019_SG_ppp_pr[dt_owin]
airbnb2019_dt_ppp_en = airbnb2019_SG_ppp_en[dt_owin]
airbnb2019_dt_ppp_sh = airbnb2019_SG_ppp_sh[dt_owin]

airbnb2021_dt_ppp_pr = airbnb2021_SG_ppp_pr[dt_owin]
airbnb2021_dt_ppp_en = airbnb2021_SG_ppp_en[dt_owin]
airbnb2021_dt_ppp_sh = airbnb2021_SG_ppp_sh[dt_owin]
airbnb2021_dt_ppp_ho = airbnb2021_SG_ppp_ho[dt_owin]
```

### 8.3.2 L-Function

The L-Function is a normalised K-Function, which measures the number of events found up to a given distance of any particular event.

### Airbnb 2019 and 2021 Listings

We will analyse the relationship of Airbnb listings by room type in 2019 and 2021 by using *Lest()* of **spatstat** package.

Similarly, we will not be doing second-order analysis for `hotel` Airbnb 2021 listingsbecause there is no such room type in Airbnb 2019 listings, so there is no point for comparison.

```{r echo=TRUE}
l_est <- function(ppp, title) {
  est <- Lest(ppp, correction = "Ripley")
  
  plot(est, . -r ~ r, ylab= "L(d)-r", xlab = "d(m)", main=title)
}
```
...
l_est(airbnb2019_dt_ppp_pr, "L_est_pr_airbnb2019")
l_est(airbnb2021_dt_ppp_pr, "L_est_pr_airbnb2021")

l_est(airbnb2019_dt_ppp_en, "L_est_en_airbnb2019")
l_est(airbnb2021_dt_ppp_en, "L_est_en_airbnb2021")

l_est(airbnb2019_dt_ppp_sh, "L_est_sh_airbnb2019")
l_est(airbnb2021_dt_ppp_sh, "L_est_sh_airbnb2021")
```
...
- **Ho**: The distribution of Airbnb 2019/2021 listings by room type (i.e. private room, entire home/apt, shared room) are randomly distributed.
- **H1**: The distribution of Airbnb 2019/2021 listings by room type (i.e. private room, entire home/apt, shared room) are **NOT** randomly distributed.

The null hypothesis will be rejected if p-value is smaller than alpha value of 0.01 (i.e. at 90% confident interval).

```{r echo=TRUE}
l_est.csr <- function(ppp, title) {
  set.seed(1234)
  
  sim <- envelope(ppp, Lest, nsim = 99, rank = 1, global=TRUE)
  
  plot(sim, . - r ~ r, xlab="d", ylab="L(d)-r", main=title)
}
```
...
l_est.csr(airbnb2019_dt_ppp_pr, "L_est_csr_pr_airbnb2019")
l_est.csr(airbnb2021_dt_ppp_pr, "L_est_csr_pr_airbnb2021")

l_est.csr(airbnb2019_dt_ppp_en, "L_est_csr_en_airbnb2019")
l_est.csr(airbnb2021_dt_ppp_en, "L_est_csr_en_airbnb2021")

l_est.csr(airbnb2019_dt_ppp_sh, "L_est_csr_sh_airbnb2019")
l_est.csr(airbnb2021_dt_ppp_sh, "L_est_csr_sh_airbnb2021")
```

#### Private Room Airbnb 2019 and 2021 Listings

- For the years 2019 and 2021, the majority of the observed (black) line does not fall within the envelope of the 90% confidence interval.
- We can therefore **reject** the null hypothesis that the distribution of `private room` Airbnb 2019/2021 listings is randomly distributed.
- We can also infer that spatial clustering is statistically significant from ~20m and ~10m for  `private room` Airbnb 2019 and 2021 listings respectively.
- Moreover, it is interesting to note that the curve reaches L(r) - r above 300 in the year 2021 while in 2019, the curve only reaches slightly about 250. This implies that the clustering pattern is greater in the year 2021 as compared to 2019.

#### Entire Home/Apt 2019 and 2021 Listings

- For the years 2019 and 2021, the majority of the observed (black) line does not fall within the envelope of the 90% confidence interval.
- We can therefore **reject** the null hypothesis that the distribution of `entire home/apt` Airbnb 2019/2021 listings is randomly distributed.
- We can also infer that spatial clustering is statistically significant from ~8m and ~10m for  `entire home/apt` Airbnb 2019 and 2021 listings. respectively.
- It is also interesting to note that both curves reach L(r) - r  ~400. This shows that clustering patterns are similar for both years.

#### Shared Room Airbnb 2019 and 2021 Listings

- For the years 2019 and 2021, the observed (black) line falls within the envelope of the 90% confidence interval.
- We therefore **cannot reject** the null hypothesis that the distribution of `shared room` Airbnb 2019/2021 listings is randomly distributed.
- Additionally, it is interesting to note that the curve only reaches L(r) - r ~400 in the year 2019 while in 2021, the curve reaches ~480. This implies that the clustering pattern is greater in the year 2019 as compared to 2021.

## 9.0 Conclusion

### 9.1 Section A

After conducting exploratory spatial data analysis, we observed that the kernel density maps of `hotels`, `MRT services` and `tourist attractions` show similar patterns to the kernel density map of `Airbnb 2019 listings`, where they are highly concentrated in the south of the Study area.

However, this is insufficient to conclude that the distribution of `Airbnb 2019 listings` are affected by the various location factors. As such, second-order spatial point pattern analysis is conducted in order for us statistically verify it.

The second-order spatial point patterns analysis reveals that the distribution of `Airbnb 2019 listings` and `MRT services` is **spatially independent**, while the distribution of `Airbnb 2019 listings` and `hotels` as well as `tourist attractions` are **NOT spatially independent**.  

As such, we can conclude that the distribution of Airbnb listings in 2019 are affected by location factors such as hotels and tourist attractions, but not MRT services.

### 9.2 Section B

After conducting exploratory spatial data analysis, we observed that the distribution of `private room` Airbnb listings decreases during the period of 2019-2021. This is in comparison to the distributions of `entire home/apt` and `shared room` Airbnb listings, which is seen to increase during the period of 2019-2021.

However, this is insufficient to conclude that the distribution of Airbnb listings are affected by COVID-19. As such, second-order spatial point pattern analysis is conducted in order for us statistically verify it.

The second-order spatial point patterns analysis reveals that the distributions of `private room` and `entire home/apt` Airbnb listings in both 2019 and 2021 are not randomly distributed. This is in comparison to the distribution of `shared room` Airbnb listings in both 2019 and 2021, where they are found to be randomly distributed. 

It was also interesting to observe that the clustering patterns of `private room` Airbnb listings are greater in 2021 than in 2019. This is in comparison to `entire home/apt` Airbnb listings, where clustering patterns are similar for both years.

Therefore, we can conclude that COVID-19 has caused some impact to the distribution of Airbnb listings, where distribution of `private room` Airbnb listings has increased during the period of 2019-2021. The distributions of Airbnb listings with other room types show little change.

## 10.0 References

- Altaweel Mark (2017, October 27). Density Mapping With GIS. Retrieved from: https://www.gislounge.com/density-mapping/
- RDocumentation (n.d.). subset.ppp: Subset of Point Pattern Satisfying A Condition. Retrieved from: https://www.rdocumentation.org/packages/spatstat/versions/1.56-1/topics/subset.ppp