...
```{r echo=TRUE}
l_cross <- function(pts, mark1, mark2) {
  l_cross <- Lcross(pts, i=mark1, j=mark2, correction='border')
  
  plot(l_cross, . -r ~ r, xlab = "distance(m)")
}
```
...
l_cross.csr(airbnb2019_factors, "airbnb2019", "hotels", 
            "Airbnb2019 Listings & Hotels")

l_cross.csr(airbnb2019_factors, "airbnb2019", "mrt",
            "Airbnb2019 Listings & MRT services")

l_cross.csr(airbnb2019_factors, "airbnb2019", "tourism",
            "Airbnb2019 Listings & Tourist Attractions")
```
...
- The distribution of `Airbnb 2019 listings` and `MRT services` are not **spatially dependent**. The null hypothesis is not **rejected** because the observed (black) line falls within the envelope of the 90% confidence interval.
