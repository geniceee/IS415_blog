...
description: |
  This in-class exercise is built on Hands-on Exercise 6, which mainly covers spatial weights and its applications. Explanations are made clearer as well.
author:
  - name: Genice Goh
    url: {}
date: 09-20-2021
...
- spdep: used to create spatial weights matrix objects and related calculations
- sf: used for importing, managing, and processing geospatial data
- tmap: used for plotting thematic maps
- tidyverse: used for importing, wrangling and visualising data
...
We are going to prepare a choropleth map showing the distribution of GDPPC 2012 by using *qtm()* of **tmap** package. In addition, we are going to visualise the counties in Hunan in the base map, so that we can get better context of the study area. 

- tm_text() is used to insert text within the map
...
We can retrieve the GDPPC of these 5 counties by using the code chunk below:
...
The printed output aboce shows that the GDPPC of the 5 nearest neighbours based on Queen's methods are 20981, 34592, 24473, 21311 and 22879.
...
- Rooks: consider areas that **share common boundaries** as neighbours
- Queen's: also include areas that **touch the tip** of the target geographical area as neighbours
...
As we can see from the top central portion of the study area, the Queen map has more links as compared to Rooks. This is due to the difference in how Queen and Rooks consider geographical areas as neighbours, which is explained earlier. 
...
TLDR: If coordinates are unprojected: **longlat=TRUE**; coordinates are projected: **longlat=FALSE**.
...
- The minimum distance is 0 so that the nearest neighbour will not look like a donut.
...
Note: Average number of links: 3.681818 means that the average number of neighbours is approximately 4.
...
The red points are the **nearest neighbours** while the black points are **distance-based neighbours** which are within the 62km defined earlier. 
...
Next, we need to assign weights to each neighboring polygon. In our case, each neighboring polygon will be assigned equal weight (style=“W”). This is accomplished by assigning the fraction **1/(# of neighbors)** to each neighboring county then **summing** the weighted income values. 
...
The **zero.policy=TRUE** option allows the weight lists to beformed with zero-length weights vectors. This should be used with **caution** since we may not be aware of missing neighbors in our dataset. 

If **zero.policy=FALSE**, it would return an error for any empty neighbour sets. Default: **zero.policy=NULL**
...
Recall in the previous section, we retrieved the GDPPC of these 5 counties by using the code chunk below:
...
We can observe an area in the central region has lower lag GDPPC as compared to its neigbours (as shown in the right graph) but in fact, it actually has higher GDPPC as compared to its neighbours (as shown in the left graph).
