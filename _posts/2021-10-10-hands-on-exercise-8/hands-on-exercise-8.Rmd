...
description: |
  In this hands-on exercise, we learn how to perform geographical segmentation by using appropriate R packages. You will also expose to R packages for performing cluster analysis and visualising clustering results.
author:
  - name: Genice Goh
    url: {}
date: 10-10-2021
...
- coorplot, ggpubr, heatmaply: used for ,ultivariate data visualisation and analysis
- cluster: used to perform cluster analysis
...
The unit of measurement of the variables is the **number of households**. Using these values directly will result in bias due to the underlying total number of households. In general, townships with higher total number of households will also have higher number of households owning radio, TV, etc.
...
Before we can prepare the choropleth map, we need to combine both the geospatial data object and aspatial data.frame object. This will be performed by using *left_join()* of **dplyr** package. 
...
In order to reveal that the number of households with radio is bias to the underlying total number of households at the township level, we will plot these two choropleth maps side by side.
...
Notice that the overall distribution of the clustering variables will change after data standardisation. Hence, it is advisable **NOT** to perform data standardisation if the **values rang**e of the clustering variables are **not very large**.
...
The agglomerative coefficients of the hierarchical clustering algorithms will now be computed.
...
After close examination of the dendrogram above, we have decided to retain five clusters.

*cutree()* of **R Base** will be used to derive a 5-cluster model. The output `groups` is a list object.
